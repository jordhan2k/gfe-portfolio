...
type DeliveryMethodProps = React.ComponentProps<'button'> & {
  name: string;
  subtitle: string;
  value: number;
  isSelected?: boolean;
...
function DeliveryMethod({
  ...
  return (
    <button
      type='button'
      onClick={onClick}
      aria-label={`Delivery method - ${name}`}
      className={cn(clsx('relative flex-1 cursor-pointer flex flex-col items-start group',
        'p-4',
        'focus:outline-none',
        'hover:bg-neutral-50'
      ), className)}
